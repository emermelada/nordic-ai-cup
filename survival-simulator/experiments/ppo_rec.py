...
CORR = np.array([0.50, 0.30], np.float32)      # heading (rad), speed (fraction of sprint_speed)
HID = 48
LOGSTD_INIT = math.log(0.05)
...
def rollout(job):
    """mode 'ppo': V2 + bounded corrections, recording transitions.
       mode 'base': pure V2 (the frozen incumbent reference), no transitions."""
    ...
    for seed in seeds:
        ...
        for i in range(horizon):
            ...
            for s in states:
                ...
                if net is not None:
                    ...
                    with torch.no_grad():          # sampling must never carry a graph
        ...
        # T = ticks actually REWARDED. On extinction the loop breaks before appending that tick's
        # reward, so i+1 would be one longer than len(REW) - that off-by-one crashed update 0.
    ...
    for upd in range(start, a.updates):
        ...
        if a.eval_every and (upd + 1) % a.eval_every == 0:
            if base_eval is None:
                bj = [("base", None, es[i:i + 4], a.eval_horizon, a.params, True)
            ...
            w2 = net_weights(net)
            pj = [("ppo", w2, es[i:i + 4], a.eval_horizon, a.params, True)
                  for i in range(0, len(es), 4)]
