...
# pkill -x, never -f: -f matches the ssh session's own command line and kills
# the shell mid-command on a rented box.
pkill -x "$(basename "$PYTHON")" 2>/dev/null
for _ in $(seq 30); do
    ss -ltn 2>/dev/null | grep -q ":$PORT " || break
    sleep 1
done
if ss -ltn 2>/dev/null | grep -q ":$PORT "; then
    echo "FAIL  port $PORT is still held; the old config would keep serving" >&2
    exit 1
fi
...
env \
  ...
  "$@" \
  "$PYTHON" api.py > "serve-$ARM.log" 2>&1 &
